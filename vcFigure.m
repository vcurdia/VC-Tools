...
% Updated: September 18, 2014 by Vasco Curdia
%
% Copyright 2013-2014 by Vasco Curdia
...
if o.RecShades.Show
    RecShadesFields = fieldnames(o.RecShades);
    RecShadesOp = {};
    for jRSO=1:(length(RecShadesFields))
        RSOj = RecShadesFields{jRSO};
        if ismember(RSOj,{'Show','Data'})
            continue
        end
        RecShadesOp{end+1} = RSOj;
        RecShadesOp{end+1} = o.RecShades.(RSOj);
    end
end
...
for jPlot=1:nPlots
  ...
  yj = y(:,:,jPlot);
  oPlot = o.Plot;
  if isfield(oPlot,'AltData')
      oPlot.AltData = oPlot.AltData(:,:,jPlot);
  end
  ...
  if isfield(o,'TitleList') && ~isempty(o.TitleList)
  ...
  if o.RecShades.Show
      vcRecessionShades(o.RecShades.Data,RecShadesOp{:})
  end
end
